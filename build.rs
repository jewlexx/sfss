...
fn write_env(mut file: &File) -> std::io::Result<()> {
    writeln!(file, "pub mod build_env {{")?;
    writeln!(file, "#![allow(unused_imports)]")?;
    writeln!(file, "// This file is autogenerated")?;

    for (key, value) in std::env::vars() {
        let key = key.replace(|c| !char::is_ascii_alphanumeric(&c), "_");
        let key = heck::AsShoutySnakeCase(key);

        writeln!(file, "pub const {key}: &str = r#\"{value}\"#;")?;
    }
